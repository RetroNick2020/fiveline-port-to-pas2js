...
begin
  ...
  begin
    ...
    QueueDrawItem(qr.x,qr.y,GBItemBrick,10);
    Delay(500);
  end;
  //DrawGameBoardItem(sx,sy,GBItemEmpty);
  QueueDrawItem(sx,sy,GBItemEmpty,10);
  
  for i:=1 to SQueueCount(FoundPath) do
  begin
    SQueueGet(FoundPath,i,qr);
  //  DrawGameBoardItem(qr.x,qr.y,Item);
    QueueDrawItem(qr.x,qr.y,Item,10);
  
    Delay(500);
//    DrawGameBoardItem(qr.x,qr.y,GBItemBrick);
    QueueDrawItem(qr.x,qr.y,GBItemBrick,5);

  end;
  //DrawGameBoardItem(tx,ty,item);
  QueueDrawItem(tx,ty,item,10);

  for i:=1 to SQueueCount(FoundPath) do
  begin
    SQueueGet(FoundPath,i,qr);
//    DrawGameBoardItem(qr.x,qr.y,GBItemEmpty);
    QueueDrawItem(qr.x,qr.y,GBItemEmpty,10);
...
begin
 ...
 QueueDrawItem(GBItemLock.x,GBItemLock.y,GB[GBItemLock.x,GBItemLock.y].Item,0);
 QueueDrawItem(GBCrossHair.x,GBCrossHair.y,GB[GBCrossHair.x,GBCrossHair.y].Item,0);
...
begin
  if k=KEY1 then PlotItem(GBItemRed);
  if k=KEY2 then PlotItem(GBItemGreen);
  if k=KEY3 then PlotItem(GBItemBrown);
  if k=KEY4 then PlotItem(GBItemCyan);
  if k=KEY5 then PlotItem(GBItemLightBlue);
  if k=KEY6 then PlotItem(GBItemLightGray);
  if k=KEY0 then PlotItem(GBItemEmpty);
end;
...
begin
  ...
  begin
    if k=LEFT then MoveCrossHairLeft;
    if k=RIGHT then MoveCrossHairRight;
    if k=UP then MoveCrossHairUp;
    if k=DOWN then MoveCrossHairDown;
  end;
